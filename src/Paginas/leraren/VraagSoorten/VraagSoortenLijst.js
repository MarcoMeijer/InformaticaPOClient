...
export default function VraagSoortenLijst({ vraagid }) {
  ...
  const [mogelijkeVraagSoorten] = useFetch("vraagsoorten", {}, (data) =>
    data.map((x) => x.vraagsoort)
  );
  const [vraagSoorten, updateVraagSoorten] = useFetch(
    "vraagsoortenvraag",
    { vraagid: vraagid },
    (data) => data.map((x) => x.vraagsoort)
  );

  const switchVraag = (vraagsoort) => {
    if (vraagSoorten.includes(vraagsoort)) {
      fetchData("ontkoppelvraagsoort", {
        vraagid: vraagid,
        vraagsoort: vraagsoort
      }).then(updateVraagSoorten);
    } else {
      fetchData("koppelvraagsoort", {
        vraagid: vraagid,
        vraagsoort: vraagsoort
      }).then(updateVraagSoorten);
    }
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
          {vraagSoorten &&
            vraagSoorten.map((vraagsoort) => (
              ...
              >
                <Text style={{ color: "#fff" }} key={vraagsoort}>
                  <b>{vraagsoort}</b>
                </Text>
                <SluitKnop
